...
**升级前自动快照** —— 启动时检测宿主版本（列车）变化：升/降级都先自动拍一份 `dsh-pre-upgrade-*` 快照再继续，"想试新版怕搞坏"的后悔药常备。首见列车只记录不拍；快照最多保留 2 份，不占 keep 配额、不进轮换；`/backup list` 在「内部快照」分区展示，可用 `/backup restore dsh-pre-upgrade-<时间>` 显式恢复。
