...
async function main() {
  ...
  if (scopedCallback === null) { ok(false, 'remote.backupPanel 作用域未激活'); }
    ...
    ok(typeof injected.panel?.status === 'function' && typeof injected.panel?.restore === 'function' && typeof injected.panel?.githubSyncNow === 'function' && typeof injected.panel?.deleteBackup === 'function' && typeof injected.panel?.setGithubRepo === 'function', '注入面提供 panel API（含 deleteBackup/github）');
