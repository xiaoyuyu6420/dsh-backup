...
const RESCUE_LAUNCHERS = Object.freeze({
  darwin: ['点我恢复.command', '#!/bin/sh\ncd "$(dirname "$0")" || exit 1\nexec node rescue.mjs serve --open\n'],
...
const PANEL_INVOCATIONS = Object.freeze([
  ...
  panelDescriptor('restore', ['selector', 'dryRun', 'types'], true),
...
class BackupPanelService extends TypertRemoteService {
  ...
  /** 恢复（dryRun 仅预览；types 非空时分类型 merge 恢复）。 */
  restore(selector, dryRun, types, signal) {
    return this.ops.restore(selector, dryRun, types, signal);
  }
...
export function apply(ctx, pluginConfig) {
   ...
   */
  /** 清理内部快照（pre-restore / pre-upgrade 共用）：只保留最近 keepN 份。 */
  async function prunePrefixedSnapshots(prefix, keepN, keepName, signal) {
    const { root } = paths();
    let dirents;
    try {
    ...
    const staleArchives = archiveCount.slice(keepN).filter((n) => n !== keepName);
  ...
  async function prunePreRestoreSnapshots(keepName, signal) {
    await prunePrefixedSnapshots('dsh-pre-restore-', 1, keepName, signal);
  }
  ...
  async function doBackup(keep, signal, opts = {}) {
    ...
    await spawnRun([tar, '--exclude=*node_modules*', '--exclude=.system', ...quarantineFlags, ...redactFlags, ...extraExcludes(), '-czf', name, '-C', parent, ...(isTyped ? typePaths : [base])], root, signal);
    ...
    const rotateStale = rotateTail.filter((b) => !tierProtectedSet(rotateTail, rotateBase[0]?.name).has(b.name)).map((b) => b.name);
  ...
  async function restoreArchive(selector, dryRun, signal, opts = {}) {
    ...
    if (Array.isArray(opts?.types) && opts.types.length) {
      ...
      try {
        const tar = await ctx.subprocess.resolveExecutable('tar');
        await spawnRun([tar, '-xzf', picked.name, '-C', parent, ...fileTargets], root, signal);
      } catch (err) {
  ...
  function summarizeDoctorScan(r) {
    ...
    return `${head}\n${detail}${more}\n\n可从备份定点修复：/backup doctor --repair [前缀|latest]（修复前会把损坏文件留档为 *.corrupt-*）`;
  }
  ...
  async function runDoctorRepair(selector, signal) {
    ...
    try {
      ...
      if (restored.size === targets.length) {
        throw new Error(`定点修复失败（${reason}）。已从留档还原全部损坏现场，数据保持修复前状态；排查磁盘空间/权限后可重试`);
      }
      const missing = targets.filter((t) => !restored.has(t.rel)).map((t) => t.rel);
      throw new Error(`定点修复失败（${reason}），且部分现场未自动还原: ${missing.join(', ')}。对应的 *.corrupt-${stamp} 留档仍在原目录旁，手工拷回原位即可`);
    }
  ...
  const panelOps = {
    ...
    backup: async (keep, types, signal) => {
      ...
      return {
        ok: true,
        summary: `备份完成: ${r.path}${validTypes ? `（类型: ${validTypes.join(', ')}${r.hasCredentials ? '，含明文凭据·不进 GitHub 同步' : ''}）` : ''}\nsha256: ${r.sha}\n轮换删除 ${r.stale} 份（保留 ${r.keep} 份）${quarantineNote(r)}`,
        path: r.path,
        sha: r.sha,
        stale: r.stale,
        keep: r.keep,
    ...
    restore: async (selector, dryRun, types, signal) => {
      try {
        const validTypes = Array.isArray(types) && types.length ? [...new Set(types.filter((t) => BACKUP_TYPES[t]))] : undefined;
        const r = await restoreArchive(selector || 'latest', Boolean(dryRun), signal, validTypes ? { types: validTypes } : {});
        if (r.dryRun) {
