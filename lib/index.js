 ...
 * - cordis.yml `config`：destination / keep / exclude（见 README）
 *
...
/** sha256 回退路径（ctx.fs.readBytes + node:crypto）的内存上限。 */
...
export function apply(ctx, pluginConfig) {
  ...
  async function listBackups() {
    ...
    return entries
      ...
      .map((e) => ({ name: e.name, size: typeof e.size === 'number' ? e.size : undefined }))
      .sort((a, b) => (a.name < b.name ? 1 : a.name > b.name ? -1 : 0));
  }

  // 校验和：POSIX 优先 sha256sum（macOS 回退 shasum）流式计算；
  // 都不可用时（Windows）回退 ctx.fs.readBytes + node:crypto 内存哈希。
  async function sha256File(absPath, home, signal) {
    ...
    let bytes;
    try {
      bytes = await ctx.fs.readBytes(await ctx.fs.resolve(absPath), signal, HASH_MAX_BYTES);
    } catch (err) {
      throw new Error(`计算 sha256 失败（文件超过 ${Math.floor(HASH_MAX_BYTES / 1048576)}MB 回退上限）: ${String(err && err.message ? err.message : err)}`);
    }
  ...
  async function verifyOne(name, home, signal) {
    ...
    let expected = '';
    try {
      const text = await ctx.fs.readText(await ctx.fs.resolve(`${archive}.sha256`), signal);
  ...
  async function restoreArchive(selector, dryRun, signal) {
    ...
    let aside = null;
    const current = await ctx.fs.stat(await ctx.fs.resolve(dshHome), signal).catch(() => undefined);
    if (current) {
  ...
  async function saveAutoState() {
    const { root } = paths();
    await ctx.fs.writeText(await ctx.fs.resolve(`${root}/auto.json`), `${JSON.stringify({ hours: autoHours })}\n`);
  }

  async function loadAutoState() {
    try {
      const { root } = paths();
      const text = await ctx.fs.readText(await ctx.fs.resolve(`${root}/auto.json`));
      const h = Number(JSON.parse(text)?.hours);
