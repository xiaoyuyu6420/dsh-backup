...
/** Settings namespace served through `ctx.settings`（持久化到 $DSH_HOME/settings.yaml）。 */
const NS = settingsNamespace('dsh-backup');

/**
 * Schema resolving the namespace; also defines the defaults.
 * Base layer: pluginConfig from cordis.patch.yml (plugin-level or profile-level).
 * User layer: runtime edits from Settings → Plugins persisted to settings.yaml.
 */
const SCHEMA = z.object({
  destination: z.string().default('~/Desktop/dsh-backups'),
  keep: z.number().min(1).default(7),
  exclude: z.array(z.string()).default([]),
  redact: z.union([z.array(z.string()), 'off']).default(SENSITIVE_DEFAULTS),
...
function resolveBase(config) {
  const out = { destination: '~/Desktop/dsh-backups', keep: 7, exclude: [], redact: [...SENSITIVE_DEFAULTS], githubRepo: '' };
...
export function apply(ctx, pluginConfig) {
  // ---------- Settings seam ----------
  // The settings service is optional: on hosts without it (non-web profiles),
  // the plugin keeps working off its `pluginConfig` base layer.
  let settingsService;
  ...
  /**
   * Resolved effective config: schema defaults ← base (cordis.patch.yml) ←
   * user layer (settings.yaml). Falls back to pluginConfig when the settings
  ...
  const panelOps = {
    ...
    setGithubRepo: async (repo) => {
      ...
      if (!raw) {
        githubState = { ...githubState, repo: null, lastError: null };
        await saveAutoState();
        // 同步清除 settings.yaml 中的 githubRepo
        if (settingsService) {
          try {
            await settingsService.update(NS, { githubRepo: '' });
          } catch (err) {
            return { ok: false, summary: String(err && err.message ? err.message : err) };
          }
        }
  ...
  // Settings thin proxy: GET/POST over the official ctx.settings seam.
  // Persists user edits to $DSH_HOME/settings.yaml; the base layer
  // (cordis.patch.yml) is never written by this plugin.
  ctx.inject(['webServer'], (scope) => {
    scope.effect(() => scope.webServer.register({
      kind: 'exact',
