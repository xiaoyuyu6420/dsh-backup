...
export function apply(ctx, pluginConfig) {
  ...
  async function restoreArchive(selector, dryRun, signal, opts = {}) {
    ...
    if (Array.isArray(opts?.types) && opts.types.length) {
      ...
      if (dryRun) {
        return {
          archive, files: mergeTargets.length, sample: mergeTargets.slice(0, 24), aside: null, snapshotPath: null, dryRun: true,
      ...
      try {
        const tar = await ctx.subprocess.resolveExecutable('tar');
        await spawnRun([tar, '-xzf', picked.name, '-C', parent, ...mergeTargets], root, signal);
      } catch (err) {
      ...
      return {
        archive, files: mergeTargets.length, aside: null, snapshotPath: null, dryRun: false,
        merge: true, types: opts.types, restored: mergeTargets.length,
