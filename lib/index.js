...
export function apply(ctx, pluginConfig) {
   ...
   * SessionHeader + 存储记录流校验。返回 null（健康）或缺陷描述字符串。
   */
  async function validateSessionFile(f) {
    try {
      const info = await fsStat(f.abs);
      if (info.size === 0) return '空文件（0 字节）';
      let lines;
      ...
      if (!lines.length) return '解出的逻辑行为空';
      return validateSessionLines(lines);
    } catch (err) {
  ...
  function summarizeDoctorScan(r) {
    ...
    const head = `扫描 ${r.scanned} 个会话日志：${r.scanned - r.corruptCount} 健康、${r.corruptCount} 损坏`;
  ...
  async function runDoctorScan(signal) {
    ...
    for (const f of found) {
      if (signal?.aborted) throw new Error('操作已取消');
      const reason = await validateSessionFile(f);
      files.push({ rel: f.rel, ok: reason === null, reason });
    }
    const corrupt = files.filter((x) => !x.ok);
    return { scanned: files.length, files, corrupt, corruptCount: corrupt.length };
  }
  ...
  async function runDoctorRepair(selector, signal) {
    ...
    // 修完复检：从归档解出的副本必须健康，否则如实报告
    const stillBad = [];
    for (const t of targets) {
      const reason = await validateSessionFile({ abs: `${dshHome}/${t.rel}`, rel: t.rel });
      if (reason !== null) stillBad.push(`${t.rel}（${reason}）`);
    }
