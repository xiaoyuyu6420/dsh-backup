...
/**
 * 探测宿主列车版本（升级前快照的触发依据）。宿主 CLI 版本没有稳定的
 * in-process API，用 peer 包 @deepseek-ai/dsh-settings 的版本做代理——
 * 它随宿主 rc 列车一起升级。多候选路径逐一尝试，全部失败返回 null
 * （特性静默停用，绝不影响装配）。
 */
...
export function apply(ctx, pluginConfig) {
  ...
  async function doBackup(keep, signal, opts = {}) {
    ...
    const stale = tail.filter((b) => !tierProtectedSet(tail, all[0]?.name).has(b.name)).map((b) => b.name);
    if (stale.length) {
