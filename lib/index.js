...
export function apply(ctx, pluginConfig) {
  ...
  async function doBackup(keep, signal) {
    ...
    try {
      ...
      if (sync.pushed) {
        githubState = { repo: githubConfig()?.repo ?? null, lastPush: sync.at, lastError: null };
        await saveAutoState();
      } else if (sync.error) {
  ...
  function githubConfig() {
    const raw = typeof pluginConfig?.githubRepo === 'string' && pluginConfig.githubRepo.trim()
  ...
  async function githubSync(signal) {
    ...
    // 镜像当前备份集：先清掉工作树里的旧归档副本（轮换删除要同步到远端）
    let staleCopies = [];
    try {
      staleCopies = (await readdir(syncDir))
        .filter((n) => n.startsWith('dsh-') && (n.endsWith('.tar.gz') || n.endsWith('.sha256')));
    } catch {
      staleCopies = [];
    }
    if (staleCopies.length) await removeFiles(staleCopies, syncDir, signal);

    // token 文件绝不能进仓库：.gitignore 防未跟踪，git rm --cached 兜底已跟踪
    await writeOwned(`${syncDir}/.gitignore`, '.git-credentials\n');
    await spawnRun([git, 'rm', '--cached', '--ignore-unmatch', '.git-credentials'], syncDir, signal);
        ...
        if (head === 'github') {
          ...
          if (arg === 'status') {
            ...
            const text = cfg
              ...
              : 'GitHub 同步未配置：在 cordis.yml 的 config.githubRepo 设置仓库（如 user/backup-repo）。';
            return { kind: 'success', text };
          }
          if (arg === 'sync') {
            const s = await githubSync(invocation.signal);
            if (s.pushed) {
              githubState = { repo: githubConfig()?.repo ?? null, lastPush: s.at, lastError: null };
              await saveAutoState();
            } else if (s.error) {
          ...
          return { kind: 'error', text: '用法: /backup github status|sync' };
        }
