...
export const inject = ['subprocess', 'fs', 'commands', 'timer'];

export function apply(ctx) {
  // ---------- 工具函数 ----------
  async function spawnRun(argv, cwd) {
  ...
  async function doBackup(keep) {
    ...
    await spawnRun([mkdir, '-p', root], home);
    ...
    await spawnRun([tar, '--exclude=*node_modules*', '--exclude=.system', '-czf', out, '-C', parent, base], home);
    ...
    const shaText = await (async () => {
      try {
        ...
        const r = await spawnRun([sha256sum, out], home);
        return r.out.trim().split(/\s+/)[0];
      } catch {
        const shasum = await ctx.subprocess.resolveExecutable('shasum');
        const r = await spawnRun([shasum, '-a', '256', out], home);
        return r.out.trim().split(/\s+/)[0];
      }
    })();
    ...
    for (const item of stale) {
      await spawnRun([rm, '-f', `${root}/${item}`, `${root}/${item}.sha256`], home);
    }
  ...
  ctx.tools.register(defineTool({
    name: 'backup_dsh',
    description: '备份或列出 DSH 用户数据（~/.dsh 的会话、配置、技能、凭据）。mode=backup 立即备份（可选 keep 指定保留份数，默认 7）；mode=list 列出已有备份与自动备份状态；mode=auto 设置自动备份（hours 为间隔小时数，0=关闭，缺省返回当前状态）。',
    parameters: {
    ...
    execute: async (args) => {
      ...
      try {
        ...
        const r = await doBackup(args && args.keep ? args.keep : undefined);
