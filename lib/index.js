 ...
 * - Settings「备份」标签页（Web）：状态、立即备份、校验、恢复、自动备份开关，
 *   经 `backupPanel` Typert Remote 命名空间访问（src-json codec，无 zod 依赖）
 *
...
import { mkdir, readdir, readFile, stat as fsStat, writeFile } from 'node:fs/promises';
...
export function apply(ctx, pluginConfig) {
  ...
  async function saveAutoState() {
    const { root } = paths();
    await writeOwned(`${root}/auto.json`, `${JSON.stringify({ hours: autoHours })}\n`);
  }

  async function loadAutoState() {
    try {
      const { root } = paths();
      const text = await readFile(`${root}/auto.json`, 'utf8');
